...
GameRenderer::GameRenderer(int virtualWidth, int virtualHeight)
...
Color GameRenderer::getElementColor(Element elem, float alpha) {
    Color c = GetElementBaseColor(elem);
    c.a = (unsigned char)(std::max(0.0f, std::min(1.0f, alpha)) * 255.0f);
    return c;
}

void GameRenderer::drawCard(Rectangle rec, Color bg, Color border, float roundness) {
    DrawRectangleRounded(rec, roundness, 8, bg);
    DrawRectangleRoundedLinesEx(rec, roundness, 8, 3.0f, border);
}

bool GameRenderer::drawButton(Rectangle rec, const char* text, Color baseColor, Color hoverColor, bool active, bool disabled, int fontSize) {
...
void GameRenderer::drawHealthBar(Vector2 pos, Vector2 size, int currentHp, int maxHp, int shield, Color fillColor) {
    // Background
...
void GameRenderer::drawStatusBadges(const std::vector<StatusInstance>& statuses, Vector2 startPos) {
    float xOffset = 0;
...
void GameRenderer::drawBackground(WeatherType weather) const {
    ...
    DrawRectangleGradientV(0, 0, m_virtualWidth, m_virtualHeight, topCol, botCol);

    // Subtle arena floor grid
    Color gridColor = (Color){ 255, 255, 255, 10 };
    for (int y = 780; y < m_virtualHeight; y += 50) {
        DrawLine(0, y, m_virtualWidth, y, gridColor);
    }
}

void GameRenderer::drawWeatherForecast(const WeatherSystem& weatherSystem) const {
    Rectangle barRec = { TOP_BAR_X, TOP_BAR_Y, TOP_BAR_W, TOP_BAR_H };
    ...
    // Top Controls
    Rectangle helpRec = { (float)m_virtualWidth - 680.0f, 35.0f, 200.0f, 80.0f };
    drawButton(helpRec, "Guide [H]", (Color){ 41, 128, 185, 255 }, (Color){ 52, 152, 219, 255 });

    Rectangle optRec = { (float)m_virtualWidth - 460.0f, 35.0f, 210.0f, 80.0f };
    drawButton(optRec, "⚙️ Options [O]", (Color){ 39, 174, 96, 255 }, (Color){ 46, 204, 113, 255 });

    Rectangle fsRec = { (float)m_virtualWidth - 230.0f, 35.0f, 190.0f, 80.0f };
...
void GameRenderer::drawPlayerPanel(const Player& player, StanceType selectedStance) const {
    Vector2 offset = player.GetRenderOffset();
    Rectangle cardRec = { PLAYER_CARD_X + offset.x, PLAYER_CARD_Y + offset.y, PLAYER_CARD_W, PLAYER_CARD_H };
...
void GameRenderer::drawEnemyPanel(const CombatSystem& combat) const {
    const auto& enemies = combat.GetEnemies();
    int selectedIdx = combat.GetSelectedTargetIndex();
    Vector2 mousePos = ScreenConfig::GetVirtualMousePosition();

    float startX = ENEMY_START_X;
    ...
    for (size_t i = 0; i < enemies.size(); ++i) {
        ...
        Rectangle rec = { startX + (float)i * (cardWidth + spacing) + offset.x, ENEMY_CARD_Y + offset.y, cardWidth, ENEMY_CARD_H };
...
void GameRenderer::drawStancePanel(const CombatSystem& combat) const {
    StanceType currentStance = combat.GetSelectedStance();
    bool isInputPhase = (combat.GetPhase() == CombatPhase::PLAYER_INPUT);

    Rectangle panelRec = { STANCE_PANEL_X, STANCE_PANEL_Y, STANCE_PANEL_W, STANCE_PANEL_H };
...
void GameRenderer::drawSkillPanel(const CombatSystem& combat) const {
    ...
    for (size_t i = 0; i < skills.size(); ++i) {
        ...
        Rectangle rec = { SKILL_TRAY_X + (float)i * (SKILL_CARD_W + spacing), SKILL_TRAY_Y, SKILL_CARD_W, SKILL_CARD_H };
...
void GameRenderer::drawExecuteButton(const CombatSystem& combat) const {
    bool isInputPhase = (combat.GetPhase() == CombatPhase::PLAYER_INPUT);
    Rectangle rec = { EXECUTE_BTN_X, EXECUTE_BTN_Y, EXECUTE_BTN_W, EXECUTE_BTN_H };
    drawButton(rec, "⚡ EXECUTE\n   TURN\n\n  [SPACE]", (Color){ 39, 174, 96, 220 }, (Color){ 46, 204, 113, 255 }, false, !isInputPhase);
}

void GameRenderer::drawLogPanel(const std::vector<CombatLogEntry>& log) const {
    Rectangle logRec = { LOG_PANEL_X, LOG_PANEL_Y, LOG_PANEL_W, LOG_PANEL_H };
...
void GameRenderer::drawSettingsOverlay(int selectedIdx) const {
    DrawRectangle(0, 0, m_virtualWidth, m_virtualHeight, (Color){ 10, 12, 18, 235 });

    Rectangle modalRec = { (float)m_virtualWidth * 0.5f - 750.0f, (float)m_virtualHeight * 0.5f - 500.0f, 1500.0f, 1000.0f };
    drawCard(modalRec, (Color){ 24, 30, 45, 255 }, (Color){ 241, 196, 15, 255 }, 0.06f);
    ...
    for (size_t i = 0; i < resList.size(); ++i) {
        ...
        drawButton(btnRec, btnText.c_str(), (Color){ 30, 38, 55, 255 }, (Color){ 41, 128, 185, 255 }, isActive);
    }
    ...
    drawButton(winBtnRec, "🗖 Windowed Mode", (Color){ 30, 38, 55, 255 }, (Color){ 52, 152, 219, 255 }, !isFs);
    drawButton(fsBtnRec, "⛶ Fullscreen Mode [F11]", (Color){ 30, 38, 55, 255 }, (Color){ 108, 92, 231, 255 }, isFs);
    ...
    drawButton(closeRec, "Apply & Close [ESC / O]", (Color){ 39, 174, 96, 255 }, (Color){ 46, 204, 113, 255 }, row2Selected);
}

void GameRenderer::drawGuideOverlay() const {
    DrawRectangle(0, 0, m_virtualWidth, m_virtualHeight, (Color){ 10, 12, 18, 230 });

    Rectangle modalRec = { (float)m_virtualWidth * 0.5f - 700.0f, (float)m_virtualHeight * 0.5f - 480.0f, 1400.0f, 960.0f };
    drawCard(modalRec, (Color){ 24, 30, 45, 255 }, (Color){ 241, 196, 15, 255 }, 0.06f);
    ...
    drawButton(closeRec, "Close Guide [H]", (Color){ 41, 128, 185, 255 }, (Color){ 52, 152, 219, 255 });
}

void GameRenderer::drawStartScreen() const {
    drawBackground(WeatherType::THUNDERSTORM);

    Rectangle titleCard = { (float)m_virtualWidth * 0.5f - 650.0f, 160.0f, 1300.0f, 980.0f };
    drawCard(titleCard, (Color){ 20, 25, 38, 245 }, (Color){ 241, 196, 15, 255 }, 0.06f);
    ...
    Rectangle startRec = { titleCard.x + titleCard.width * 0.5f - 260.0f, titleCard.y + 540.0f, 520.0f, 85.0f };
    drawButton(startRec, "START EXPEDITION [ENTER]", (Color){ 39, 174, 96, 255 }, (Color){ 46, 204, 113, 255 }, true);

    Rectangle optRec = { titleCard.x + titleCard.width * 0.5f - 260.0f, titleCard.y + 640.0f, 520.0f, 75.0f };
    drawButton(optRec, "⚙️ VIDEO OPTIONS (UP TO QHD) [O]", (Color){ 41, 128, 185, 255 }, (Color){ 52, 152, 219, 255 });

    Rectangle fsRec = { titleCard.x + titleCard.width * 0.5f - 260.0f, titleCard.y + 730.0f, 520.0f, 75.0f };
    bool isFs = IsWindowFullscreen();
...
void GameRenderer::drawVictoryOverlay(const CombatSystem& combat) const {
    DrawRectangle(0, 0, m_virtualWidth, m_virtualHeight, (Color){ 10, 15, 25, 220 });

    Rectangle card = { (float)m_virtualWidth * 0.5f - 500.0f, (float)m_virtualHeight * 0.5f - 350.0f, 1000.0f, 700.0f };
    drawCard(card, (Color){ 20, 28, 42, 255 }, (Color){ 46, 204, 113, 255 }, 0.06f);
    ...
    Rectangle nextRec = { card.x + card.width * 0.5f - 220.0f, card.y + 480.0f, 440.0f, 90.0f };
...
void GameRenderer::drawGameOverOverlay(const CombatSystem& combat) const {
    (void)combat;
    DrawRectangle(0, 0, m_virtualWidth, m_virtualHeight, (Color){ 25, 10, 15, 230 });

    Rectangle card = { (float)m_virtualWidth * 0.5f - 500.0f, (float)m_virtualHeight * 0.5f - 350.0f, 1000.0f, 700.0f };
    drawCard(card, (Color){ 32, 20, 25, 255 }, (Color){ 231, 76, 60, 255 }, 0.06f);
    ...
    Rectangle retryRec = { card.x + card.width * 0.5f - 220.0f, card.y + 480.0f, 440.0f, 90.0f };
    drawButton(retryRec, "TRY AGAIN [ENTER]", (Color){ 192, 57, 43, 255 }, (Color){ 231, 76, 60, 255 }, true);
}

void GameRenderer::render(const CombatSystem& combat, const ParticleSystem& particles, GameScene scene, int selectedSettingIdx, bool showGuide, bool showSettings) {
    (void)particles;
    switch (scene) {
        ...
        case GameScene::BATTLE:
            ...
            drawPlayerPanel(combat.GetPlayer(), combat.GetSelectedStance());
            drawEnemyPanel(combat);
            drawStancePanel(combat);
            drawSkillPanel(combat);
            drawExecuteButton(combat);
            drawLogPanel(combat.GetCombatLog());
        ...
        case GameScene::VICTORY:
            ...
            if (showSettings) drawSettingsOverlay(selectedSettingIdx);
            break;
